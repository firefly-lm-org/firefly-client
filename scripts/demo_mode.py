...
def run_demo(domain: str = "law", num_nodes: int = 4, steps: int = 5, output_dir: str = "./demo_output") -> bool:
    ...
    # ── Step 7: Teaching Report ──
    cheat_passed = len(suspicious) == 0
    print("\n" + "=" * 60)
    print("  Firefly LM Demo 完成!")
    print("  你的第一份联邦学习实验报告")
    print("=" * 60)
    print()
    print("  你刚刚体验了完整的联邦学习流程:")
    print(f"  1. 数据准备: {num_nodes} 个节点各分配到 {items_per_node} 条 {domain} 数据")
    print(f"  2. 本地训练: 每个节点独立训练了 {steps} 步")
    print(f"  3. 防作弊检测: {'全部通过' if cheat_passed else '发现可疑节点'}")
    print(f"  4. 安全聚合: {num_nodes} 个适配器加权合并为 1 个联邦模型")
    print(f"  5. 效果评估: 聚合模型在 holdout 集上测试")
    print()
    print("  各节点贡献:")
    for nr in node_results:
        print(f"    Node {nr['node_id']}: loss={nr['final_loss']:.4f}, samples={nr['sample_count']}")
    print()
    print(f"  聚合模型: {output_dir}/aggregated.safetensors")
    print(f"  训练耗时: {train_time:.1f}s")
    print(f"  SHA256: {agg_sha[:16]}...")
    print()
    print("  核心理解:")
    print("  - 每个节点的数据从未离开本机")
    print("  - 聚合的是模型权重, 不是原始数据")
    print("  - 防作弊系统确保没有人上传垃圾权重")
    print()
    print("  下一步:")
